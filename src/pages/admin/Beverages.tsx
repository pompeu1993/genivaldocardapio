...
import { Trash2, Plus, Loader2, Wine, Edit2, Wand2, ImageIcon } from "lucide-react"
  ...
  async function fetchData() {
    try {
      ...
      if (categoriesRes.data) {
        ...
        const beverageCats = categoriesRes.data.filter(c => 
        ...
        if (dishesRes.data) {
            // Filter dishes that belong to beverage categories
            const beverageCatIds = beverageCats.map(c => c.id)
            const beverageDishes = dishesRes.data.filter(d => beverageCatIds.includes(d.category_id))
            setDishes(beverageDishes)
        }
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold font-serif text-secondary">Gerenciar Bebidas</h1>
        <div className="flex gap-4">
            <select
                className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            ...
            <Button onClick={() => { setEditingDish(null); setIsModalOpen(true); }}>
      ...
      {isModalOpen && (
        <Card className="mb-8 border-secondary/20 bg-secondary/5">
          <CardHeader>
            <CardTitle>{editingDish ? "Editar Bebida" : "Adicionar Nova Bebida"}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button type="submit" disabled={saving || uploading}>
                  ...
                  {editingDish ? "Salvar Alterações" : "Salvar Bebida"}
                </Button>
              </div>
            </form>
          </CardContent>
